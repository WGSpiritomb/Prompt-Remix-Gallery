...
/**
 * Determines whether a preset is already a mixed fusion or combined preset.
 * Per requirement: "dont use already mixed fusions for mixing".
 * Excludes any preset that is a combined style, 2-mix fusion, or multi-style blend.
 */
export function isAlreadyMixedFusion(preset: StylePreset | null | undefined): boolean {
  if (!preset) return false;
  // Explicitly marked as combined (e.g. style combiner output or multi-mix)
  if (preset.isCombined === true) return true;
  // Has parent style IDs or names recorded
  if (Array.isArray(preset.parentStyleIds) && preset.parentStyleIds.length > 0) return true;
  if (Array.isArray(preset.parentStyleNames) && preset.parentStyleNames.length > 0) return true;
  // ID prefix generated by fusion combiners
  if (typeof preset.id === 'string' && preset.id.startsWith('fusion_')) return true;
  // Matches 2-mix fusion heuristic
  if (isTwoMixFusion(preset)) return true;
  // Matches fusion/blend naming pattern (e.g., "Kuuki-01 + Topu-01 Fusion" or "X + Y Blend")
  if (/\s+\+\s+.*(?:fusion|blend|mix)/i.test(preset.name)) return true;
  return false;
}

/**
 * Filter out any presets that should be ignored for mixing:
 * 1. Base styles with leading numbers (e.g. 01-Base)
 * 2. Already mixed fusions (e.g. Kuuki-01 + Topu-01 Fusion)
 */
export function filterMixablePresets(presets: StylePreset[]): StylePreset[] {
  return presets.filter(
    (p) => !isBaseStyleIgnoredForMixing(p) && !isAlreadyMixedFusion(p)
  );
}
 ...
 * 2. Neither style can be an already mixed fusion (per "dont use already mixed fusions for mixing")
 * 3. They cannot be the same style
 * 4. They must not share any derived artists
 */
export function canStylesBeMixed(
  ...
  if (isAlreadyMixedFusion(presetA)) {
    return {
      canMix: false,
      reason: `"${presetA.name}" is already a mixed fusion and cannot be used for mixing.`,
    };
  }
  if (isAlreadyMixedFusion(presetB)) {
    return {
      canMix: false,
      reason: `"${presetB.name}" is already a mixed fusion and cannot be used for mixing.`,
    };
  }
...
/**
 * Filter presets to find only those compatible with target (i.e. having no common artists, not ignored Base styles, and not already mixed fusions)
 */
export function getCompatibleStylesFor(target: StylePreset, presets: StylePreset[]): StylePreset[] {
  if (isBaseStyleIgnoredForMixing(target) || isAlreadyMixedFusion(target)) return [];
  return presets.filter(
    (p) =>
      p.id !== target.id &&
      !isBaseStyleIgnoredForMixing(p) &&
      !isAlreadyMixedFusion(p) &&
...
export function createCombinedPreset(
  ...
  if (isAlreadyMixedFusion(styleA)) {
    throw new Error(
      `Cannot combine styles: "${styleA.name}" is already a mixed fusion and cannot be used for mixing.`
    );
  }
  if (isAlreadyMixedFusion(styleB)) {
    throw new Error(
      `Cannot combine styles: "${styleB.name}" is already a mixed fusion and cannot be used for mixing.`
    );
  }
